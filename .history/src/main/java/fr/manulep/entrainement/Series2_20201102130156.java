...
public class Series2 {
	...
	public static boolean isOrder(int... number) {
		...
		// Notre compareArray recoit les mêmes données que notre emptyArray
		//compareArray = emptyArray; 
		
		// On trie dans l'ordre croissant notre compare Array
		Collections.sort(compareArray);
		//System.out.println(compareArray);
		//System.out.println(emptyArray);
		if(emptyArray == compareArray){
			result = true; 
		}else{
			result = false; 
		}
		// for (Integer numCroi : emptyArray) {
		// 	if(emptyArray == compareArray){
		// 		result = true; 
		// 	}
		// }
		// croissant
		/*if(emptyArray.get(0) < emptyArray.get(1)){
			for (Integer numCroi : emptyArray) {
				
				System.out.println(numCroi);
			} 
		/*}else if(emptyArray.get(0) > ){
