...
public class Series1 {
	...
	public static String[] selectElementsStartingWithA(String[] array) {
		...
		int nb = 0; //on initialise nb pour qu'il parte de 0, sinon il y'aura une error
		for (String elem : array) { // pour chaque element de mon array
			if(elem.charAt(O) == 'a'){ // si l'élément commence par 'a' à l'index 0
				nb++; // alors incrémente pour passer d'élements en éléments
		...
		arrayA = new String[nb]; 

		// remplir le tableau (2eme version)
		...
		for (int i = 0; i < array.length; i++) {
			...
			if(elem.charAt(O) == 'a') { //si nos élements commencent par 'a' à leur index 0 
